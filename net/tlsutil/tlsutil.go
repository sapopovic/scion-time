...
func createCertificate(priv *rsa.PrivateKey) (*tls.Certificate, error) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"scion-time"},
		},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
	...
	err = pem.Encode(certPEMBuf, &pem.Block{Type: "CERTIFICATE", Bytes: certBytes})
	if err != nil {
		return nil, err
	}
	...
	err = pem.Encode(keyPEMBuf, &pem.Block{Type: "PRIVATE KEY", Bytes: keyBytes})
	if err != nil {
		return nil, err
	}
