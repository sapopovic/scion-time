...
type ReferenceClock interface {
	...
	Drift() (drift time.Duration, ok bool)
}
...
func ClockDriftIP(ntpc *IPClient) (time.Duration, bool) {
	var d time.Duration
...
func ClockDriftSCION(ntpcs []*SCIONClient) (time.Duration, bool) {
	...
	return timemath.Median(ds), true
}
