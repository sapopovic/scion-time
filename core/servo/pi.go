...
func (s *PiServo) Sample(offset int64, localTs uint64) (float64, State) {
	...
	case 1:
		...
		if localDiff < freqEstInterval {
			slog.Debug("servo Sample is called too often, not enough time passed since first sample")
			break
		}
...
func (f *PiServoFilter) isSpike(offset int64, lastCorrection time.Time) filterState {
	...
	slog.Debug("Filter.isSpike", "offset stdev", f.offsetStdev, "wait factor", waitFactor, "max offset locked", maxOffsetLocked)
