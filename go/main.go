...
func runBenchmark(daemonAddr string, localAddr snet.UDPAddr, remoteAddr snet.UDPAddr) {
	var err error
	ctx := context.Background()
...
func main() {
	...
	clientFlags.BoolVar(&enableBenchmark, "benchmark", false, "Benchmark")
	...
	case "client":
		...
		log.Print("enableBenchmark:", enableBenchmark)
		if !enableBenchmark {
			runClient(daemonAddr, localAddr, remoteAddr)
		} else {
			runBenchmark(daemonAddr, localAddr, remoteAddr)
		}
	default:
		exitWithUsage()
	}
